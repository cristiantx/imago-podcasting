...
export const ingestJobs = pgTable(
  "ingest_jobs",
  {
    ...
    queueDispatchStatus: varchar("queue_dispatch_status", { length: 24 }).notNull().default("pending"),
    queueDispatchAttempts: integer("queue_dispatch_attempts").notNull().default(0),
    queueDispatchError: text("queue_dispatch_error"),
    queuedEpisodeIds: jsonb("queued_episode_ids").$type<string[]>().notNull().default([]),
